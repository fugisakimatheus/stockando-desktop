...
- [ ] 1. Database layer and migration infrastructure
  - [ ] 1.1 Create the `appSettings` table in `src/main/db/schema.ts` and add Drizzle type exports
  ...
  - [ ] 1.2 Implement the Migration Runner in `src/main/db/migrations/`
  ...
  - [ ] 1.3 Create the initial migration file (`001-initial-schema`)
  ...
  - [ ] 1.4 Implement seed data logic for first-run initialization
  ...
  - [ ]* 1.5 Write property test for migration sequential ordering
    - **Property 2: Migration sequential ordering**
    - **Validates: Requirements 6.2**

  - [ ]* 1.6 Write property test for migration transactional atomicity
...
- [ ] 3. Fastify HTTP server refactoring and bootstrap sequence
  - [ ] 3.1 Refactor `src/main/server.ts` into a proper bootstrap sequence
  ...
  - [ ] 3.2 Implement the standard API response envelope and error handling
  ...
  - [ ] 3.3 Implement the bootstrap API route (`GET /api/bootstrap`)
...
- [ ] 4. Company management API routes
  - [ ] 4.1 Implement `GET /api/companies` and `POST /api/companies`
  ...
  - [ ]* 4.4 Write property test for company data isolation
    - **Property 1: Company data isolation**
    - **Validates: Requirements 2.4**

  - [ ]* 4.5 Write property test for company name uniqueness enforcement
    - **Property 6: Company name uniqueness enforcement**
    - **Validates: Requirements 3.2**

- [ ] 5. Application settings API routes
  - [ ] 5.1 Implement `GET /api/settings` and `PUT /api/settings`
    - Read all app-level settings as key-value pairs
    - Write app-level settings atomically
    - Return defaults if no settings exist
    - _Requirements: 4.1, 4.2, 4.5_

  - [ ] 5.2 Implement `PUT /api/settings/active-company`
    - Set the last active company ID in app settings
    - Validate the company ID exists before persisting
    - _Requirements: 2.6_

  - [ ]* 5.3 Write property test for settings two-tier resolution
    - **Property 4: Settings two-tier resolution**
    - **Validates: Requirements 4.1, 4.5**

  - [ ]* 5.4 Write property test for settings write atomicity
...
- [ ] 7. Shared API client and query hooks in the renderer
  - [ ] 7.1 Create the shared API client in `src/renderer/src/shared/api/client.ts`
  ...
  - [ ] 7.2 Create company query hooks in `src/renderer/src/shared/hooks/use-companies.ts`
  ...
  - [ ] 7.3 Create settings query hooks in `src/renderer/src/shared/hooks/use-settings.ts`
  ...
  - [ ] 7.4 Create the active company hook in `src/renderer/src/shared/hooks/use-active-company.ts`
  ...
  - [ ] 7.5 Create the bootstrap hook in `src/renderer/src/shared/hooks/use-bootstrap.ts`
    - `useBootstrap()` — calls `GET /api/bootstrap` on app start
    - Returns bootstrap status, last active company, and error state
    - _Requirements: 1.1, 1.2, 1.3_

- [ ] 8. App shell enhancements and company context UI
  - [ ] 8.1 Add company selector to the sidebar header in `AppShell`
  ...
  - [ ] 8.2 Implement the company creation flow
  ...
  - [ ] 8.3 Implement bootstrap error and empty states in the app shell
...
- [ ] 9. Settings page implementation
  - [ ] 9.1 Build the settings page at `src/renderer/src/pages/settings/`
  ...
  - [ ] 9.2 Add save feedback and error handling to settings forms
...
- [ ] 10. Navigation and routing enhancements
  - [ ] 10.1 Add lazy loading to route definitions in `src/renderer/src/app/router.tsx`
  ...
  - [ ] 10.2 Create the NotFound page at `src/renderer/src/pages/not-found/`
...
- [ ] 11. Shared UI primitives
  - [ ] 11.1 Implement layout primitives: `PageShell` and `Section` in `src/renderer/src/shared/ui/`
  ...
  - [ ] 11.2 Implement state primitives: `EmptyState`, `ErrorState`, `LoadingState`
  ...
  - [ ] 11.3 Verify existing form, table, and dialog primitives for Phase 0 completeness
...
- [ ] 12. Audit metadata and timestamp handling
  - [ ] 12.1 Implement audit timestamp utilities in `src/main/lib/timestamps.ts`
  ...
  - [ ]* 12.2 Write property test for audit timestamp consistency
    - **Property 5: Audit timestamp consistency**
    - **Validates: Requirements 12.2, 12.3**

- [ ] 13. Integration testing and final wiring
  - [ ] 13.1 Wire the bootstrap hook into the app entry point
  ...
  - [ ]* 13.2 Write integration tests for the bootstrap sequence
  ...
  - [ ]* 13.3 Write integration tests for company CRUD through the API
  ...
  - [ ]* 13.4 Write integration tests for settings persistence and cache invalidation
