  ...
  return (
    <DialogOverlay isDismissable={isDismissable} {...props}>
      <ModalPrimitive
        ...
        className={cn(
          'fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl border border-border/70 bg-popover/95 p-4 text-sm text-popover-foreground shadow-[0_20px_60px_rgba(15,23,42,0.18)] ring-1 ring-foreground/10 backdrop-blur-xl duration-100 outline-none data-entering:animate-in data-entering:fade-in-0 data-entering:zoom-in-95 data-exiting:animate-out data-exiting:fade-out-0 data-exiting:zoom-out-95 sm:max-w-sm dark:border-white/10 dark:bg-popover/90 dark:shadow-[0_20px_60px_rgba(2,6,23,0.3)]',
          className
        )}
      >
...
function DialogHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="dialog-header" className={cn('flex flex-col gap-1.5 pb-1', className)} {...props} />
}
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        '-mx-4 -mb-4 flex flex-col-reverse gap-2 rounded-b-2xl border-t border-border/70 bg-gradient-to-br from-primary/3 via-primary/2 to-primary/1 p-4 sm:flex-row sm:justify-end dark:border-white/10',
        className
      )}
      {...props}
    >
