...
function ComboboxItem<T extends object>({ className, children, ...props }: ListBoxItemProps<T>) {
  return (
    <ListBoxItemPrimitive
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-xl py-1 pr-8 pl-1.5 text-sm outline-hidden select-none data-focused:bg-accent/80 data-focused:text-accent-foreground not-data-[variant=destructive]:data-focused:**:text-accent-foreground data-highlighted:bg-accent/80 data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
function ComboboxChip({
  ...
  return (
    <TagPrimitive
      ...
      className={cn(
        'flex h-[calc(--spacing(5.25))] w-fit items-center justify-center gap-1 rounded-xl border border-border/70 bg-muted/80 px-1.5 text-xs font-medium whitespace-nowrap text-foreground shadow-[0_2px_8px_rgba(15,23,42,0.04)] has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0 dark:border-white/10 dark:bg-muted/70 dark:shadow-[0_2px_8px_rgba(2,6,23,0.16)]',
        className
      )}
      {...props}
    >
